...
2. Build a temporary Kimi home containing the managed OAuth credential link, filtered provider/model config and a minimal `.gitconfig`. Refuse enabled project-local Kimi MCP configuration.
3. Spawn the installed `kimi acp` without a shell, with `KIMI_CODE_HOME`, `HOME` and `USERPROFILE` pointing at that temporary home, `BASH_ENV` at the guard bootstrap and `KIMI_SHELL_PATH` at the resolved bash.
